...
export default function ParentDashboard() {
  ...
  const stableInsights = recentSessions.length >= 3 && evaluatedSkills >= 12
  const strongest = stableInsights ? strongestSkills(skills, evidence) : []
  const reinforcement = stableInsights ? reinforcementSkills(skills, evidence) : []
  ...
  const subjectSummaries = ACTIVE_SUBJECT_IDS.map((subjectId) => {
    ...
    const practiced = ids.filter((id) => Number(evidence.attempts[id] ?? 0) > 0).length
    const sampleReady = attempts >= 10 && practiced >= Math.min(3, ids.length)
    ...
    return {
      ...
      attempts,
      sampleReady,
      accuracy: sampleReady ? Math.round((correct / attempts) * 100) : null,
      mastery: sampleReady && subjectMastery.evidence ? subjectMastery.value : null,
    }
  })
  ...
  return <main className="shell">
    ...
    <section className="card"><span className="tag">🧭 VISIÓN POR ASIGNATURA</span><h2>Evidencia en cada materia</h2><div className="grid two">{subjectSummaries.map((summary) => { const def = subjectDefinition(summary.subjectId); return <div className="metric" key={summary.subjectId}><b>{def?.icon ?? '📘'} {def?.name ?? summary.subjectId}</b><p className="muted">{summary.practiced}/{summary.total || '—'} habilidades · {summary.attempts} respuestas</p><p className="muted">{summary.sampleReady ? <>Precisión {summary.accuracy}% · dominio {summary.mastery === null ? '—' : `${summary.mastery}%`}</> : summary.attempts ? 'Muestra inicial · todavía sin diagnóstico por materia' : 'Sin práctica todavía'}</p></div> })}</div></section>
