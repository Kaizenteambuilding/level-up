...
export default function GameOnboarding() {
  ...
  return <section className="onboarding-stage" aria-labelledby="onboarding-title">
    ...
    {step === 'reward' && <div className="onboarding-panel reward-panel"><span className="tag">PASO 4 · MISIÓN COMPLETADA</span><h1 id="onboarding-title">El mundo ya reconoce tu progreso</h1><div className="reward-burst" aria-hidden="true">🏆</div><p>Has completado la calibración. Ya puedes explorar Matemáticas, Lengua, Inglés, Ciencias y Geografía/Historia desde el mapa.</p><DemoHud player={player} game={game} />{message && <p className="status" role="alert">{message}</p>}<button className="btn primary" type="button" disabled={saving} onClick={finishOnboarding}>{saving ? 'ABRIENDO MUNDO…' : 'ENTRAR EN MI MUNDO'}</button><Link href="/mission/briefing?onboarding=1" className="btn dark">REPETIR MISIÓN GUIADA</Link></div>}
  </section>
}
