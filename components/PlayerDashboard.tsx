...
export default function PlayerDashboard() {
  ...
  async function load() {
    ...
    if(openSessionError){warnings.push('No se pudo comprobar si hay una misión guardada.');setOpenMissionProgress(null);setOpenMissionProgressUnknown(false)} else if(openSession){const {count,error:countError}=await supabase.from('attempts').select('id',{count:'exact',head:true}).eq('session_id',openSession.id); if(countError||count===null){warnings.push('No se pudo cargar el avance exacto de la misión guardada.');setOpenMissionProgress(null);setOpenMissionProgressUnknown(true)} else {setOpenMissionProgress(Math.min(SESSION_LENGTH,Number(count)));setOpenMissionProgressUnknown(false)}} else {setOpenMissionProgress(null);setOpenMissionProgressUnknown(false)}
    const [{data:activityData,error:activityError},{data:sessionData,error:sessionError},{data:stateData,error:stateError},{data:skillEvidenceData,error:skillEvidenceError}]=await Promise.all([
      ...
      supabase.from('study_sessions').select('id,started_at,ended_at,completed,xp_earned').eq('player_id',currentPlayer.id).eq('completed',true).eq('phase','done').order('started_at',{ascending:false}).limit(1).maybeSingle(),
    ...
    if(sessionError) warnings.push('No se pudo cargar la última misión completada.'); else if(sessionData){setLastSession(sessionData as LastSession);const {data:attemptsData,error:attemptsError}=await supabase.from('attempts').select('correct').eq('session_id',sessionData.id);if(attemptsError)warnings.push('No se pudieron cargar los resultados de la última misión.');const attempts=attemptsError?[]:attemptsData??[];setSessionAttempts(attempts.length);setSessionCorrect(attempts.filter((a:any)=>a.correct===true).length)}
  ...
  const missionLabel=hasOpenMission?(openMissionProgressUnknown?'▶ CONTINUAR MISIÓN':`▶ CONTINUAR MISIÓN · ${openMissionProgress}/${SESSION_LENGTH}`):completedToday?'✓ MISIÓN DE HOY COMPLETADA':'▶ MISIÓN DE HOY'
  ...
  return <>
    <section className="card hero"><span className="tag">🎮 PARTIDA DE HOY</span><h1>{player.alias}</h1><p className="muted">Nivel {gameLevel.level} · {gameRank(gameLevel.level)} · {player.xp} XP · 🪙 {player.coins} · 🔥 {streak} días de racha</p>
      {hasOpenMission&&<div className="metric" style={{marginTop:14}}><b>{openMissionProgressUnknown?'⏯ Misión en curso':`⏯ Misión en curso · ${openMissionProgress}/${SESSION_LENGTH}`}</b><p className="muted" style={{marginBottom:0}}>{openMissionProgressUnknown?'Tu misión está guardada. No se ha podido leer el progreso exacto en este momento.':'Tu progreso está guardado. Puedes continuar exactamente donde lo dejaste.'}</p></div>}
      {completedToday&&<div className="metric" style={{marginTop:14}}><b>✅ Entrenamiento diario completado</b><p className="muted" style={{marginBottom:0}}>Tu misión de hoy ya está guardada. Explora el mundo, revisa tus recompensas y vuelve mañana para una nueva misión.</p></div>}
    ...
    <section className="card"><span className="tag">📊 ÚLTIMA MISIÓN</span><h2>Tu última partida</h2>{lastSession?<div className="grid two"><div className="metric"><b>{accuracy}%</b><p className="muted">{sessionCorrect}/{sessionAttempts} respuestas correctas</p></div><div className="metric"><b>+{lastSession.xp_earned} XP</b><p className="muted">ganados en la última sesión</p></div></div>:<p className="muted">Completa tu primera misión para ver aquí tus resultados.</p>}</section>
