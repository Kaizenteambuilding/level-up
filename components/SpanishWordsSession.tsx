...
export default function SpanishWordsSession() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const { data: opened, error: openError } = await withTimeout(openPractice('open_levelup_practice_session', { p_player_id: id, p_mode: MODE }), 'La apertura de Taller de palabras')
        if (!active) return
        if (openError) {
          if ((openError.message ?? '').toLowerCase().includes('practice already completed today')) { setCompletedToday(true); return }
          throw new Error(userFacingError(openError, 'No se pudo abrir Taller de palabras.'))
        }
        ...
        if (attemptsResult.error) throw new Error(userFacingError(attemptsResult.error, 'No se pudo recuperar el taller.'))
        ...
        const { data: skillRows, error: skillsError } = await withTimeout(supabase.from('skills').select('id,name,generator_key,unit_id').eq('active', true).in('unit_id', unitIds), 'La carga de gramática y vocabulario')
        if (!active) return
        if (skillsError || !skillRows?.length) throw new Error('No hay habilidades activas de Lengua disponibles')
        const loadedSkills = (skillRows as SkillRow[]).filter((skill) => WORD_SKILL_IDS.has(skill.id))
        if (!loadedSkills.length) throw new Error('No hay habilidades de gramática y vocabulario disponibles')
        ...
        for (const row of statesResult.data ?? []) {
          ...
          if (!WORD_SKILL_IDS.has(skillId)) continue
      ...
      } catch (cause) { if (active) setError(errorMessage(cause, 'No se pudo abrir Taller de palabras.')) }
  ...
  useEffect(() => {
    ...
    if (!primarySkill) { setError('No se pudo elegir un reto de palabras.'); return }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const supabase = createSupabaseBrowserClient()
        if (!supabase) throw new Error('No se pudo conectar para cerrar el taller')
        const { data, error: closeError } = await withTimeout(supabase.rpc('complete_levelup_session', { p_session_id: sessionId }), 'El cierre de Taller de palabras')
        if (closeError) throw new Error(userFacingError(closeError, 'No se pudo cerrar Taller de palabras.'))
        const result = data as { completed?: boolean; attempts?: number; xp_earned?: number }
        if (!result?.completed || Number(result.attempts) !== SESSION_LENGTH) throw new Error('El servidor no confirmó los 10 retos de palabras')
        setXp(Number(result.xp_earned ?? xp)); setCompleted(true)
      } catch (cause) { setError(errorMessage(cause, 'No se pudo cerrar Taller de palabras.')) }
  ...
  async function submit(optionIndex: number) {
    ...
    try {
      ...
      setFeedback((ok ? '✓ Palabra dominada' : '↻ Revisa la regla o el significado') + ` · ${Number(result.xp_awarded ?? 0)} XP`)
  ...
  if (index >= SESSION_LENGTH) {
    ...
    return <section className="card" style={{ textAlign: 'center', padding: 45 }}><div style={{ fontSize: 72 }}>🧩✨</div><span className="tag">TALLER SUPERADO</span><h1>Archivo de palabras reconstruido</h1><p className="muted">10 retos · {Math.round((correct / SESSION_LENGTH) * 100)}% precisión · +{xp} XP</p><div className="action-row" style={{ justifyContent: 'center' }}><Link className="btn primary" href="/zone/language">VOLVER A LA BIBLIOTECA</Link><Link className="btn dark" href="/world">VOLVER AL MUNDO</Link></div></section>
  }
  if (!question) return <section className="card" role="status"><p className="muted">Preparando el siguiente reto de palabras…</p></section>

  const progress = Math.min(SESSION_LENGTH, index + Number(answered && feedback))
  return <div className="mission-console"><section className="card mission-control"><div><span className="tag">📚 LENGUA · TALLER DE PALABRAS</span><h1>Reto {index + 1} de {SESSION_LENGTH}</h1><p className="muted">Gramática, sintaxis, vocabulario y formación de palabras adaptados al progreso real.</p></div><Link className="btn dark mission-exit" href="/zone/language">SALIR A LA BIBLIOTECA</Link><div className="energy-track" role="progressbar" aria-valuemin={0} aria-valuemax={SESSION_LENGTH} aria-valuenow={progress}>{Array.from({ length: SESSION_LENGTH }, (_, position) => <span key={position} className={position < progress ? 'charged' : position === index ? 'active' : ''}>{position < progress ? '⚡' : position + 1}</span>)}</div></section><div className="mission-stage"><aside className="mission-core"><span className="mission-avatar">🧩</span><span className="core-city">📚</span><div className="core-orb">⚡</div><b>Taller {Math.round((progress / SESSION_LENGTH) * 100)}%</b><span>{correct} aciertos · {xp} XP</span><small>10 retos para reconstruir el archivo</small></aside><section className="card mission-question"><span className="tag">{question.label} · dificultad {question.difficulty}/5</span><p tabIndex={-1} style={{ fontSize: 24, fontWeight: 900 }}>{question.prompt}</p><div className="answers">{question.options.map((option, i) => { const isCorrect = answered && i === question.answerIndex; const isWrong = answered && i === selectedOption && !isCorrect; return <button key={i} className={`answer${isCorrect ? ' correct' : ''}${isWrong ? ' incorrect' : ''}`} disabled={answered} type="button" onClick={() => submit(i)}>{String.fromCharCode(65 + i)} · {option}{isCorrect ? ' · ✓ Correcta' : isWrong ? ' · ✕ Tu respuesta' : ''}</button> })}</div>{feedback && <div className="metric" style={{ marginTop: 16 }}><b>{feedback}</b></div>}{answered && feedback && <><div className="metric" style={{ marginTop: 10 }}><b>💡 Regla y significado</b><p className="muted" style={{ marginBottom: 0 }}>{question.solution}</p></div><button className="btn primary" type="button" style={{ marginTop: 14 }} onClick={next}>{index + 1 === SESSION_LENGTH ? 'VALIDAR TALLER' : 'SIGUIENTE RETO'}</button></>}</section></div></div>
}
