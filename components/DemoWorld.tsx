...
export default function DemoWorld() {
  ...
  const cadenceHref = cadence.status === 'done-today' ? '/zone/math' : '/mission/briefing'
  const weeklyQuestAction = (questId: string) => {
    ...
    return { href: '/mission/briefing', label: 'HACER MISIÓN' }
  }
    ...
    <>
      <section className="game-world" aria-labelledby="world-title">
        ...
        <div className="world-map">
          <article className="world-zone math-zone unlocked"><span className="zone-icon" aria-hidden="true">🎯</span><span className="zone-state">{cadence.status === 'done-today' ? '✓ MISIÓN DE HOY COMPLETADA' : cadence.status === 'returning' ? 'NUEVA MISIÓN DISPONIBLE' : 'MISIÓN DIARIA ACTIVA'}</span><h2>Expedición diaria</h2><p>{cadence.message}</p><div className="action-row"><Link href={cadenceHref} className="btn primary">▶ {cadence.action}</Link><Link href="/zone/math" className="btn dark">EXPLORAR MATEMÁTICAS</Link><Link href="/player" className="btn dark">VER PROGRESO</Link></div></article>
          {WORLD_ZONES.map((zone) => <Link href={`/zone/${zone.id}`} className={`world-zone preview-zone ${zone.className} unlocked`} key={zone.name}><span className="zone-icon" aria-hidden="true">{zone.icon}</span><span className="zone-state">✓ 3 DISTRITOS ACTIVOS</span><h2>{zone.name}</h2><p>{zone.tagline}. Entra y elige una misión.</p></Link>)}
          {player.level >= 2 ? <Link href="/shop" className="world-zone shop-zone unlocked"><span className="zone-icon" aria-hidden="true">🏪</span><span className="zone-state">ABIERTO · NIVEL 2</span><h2>Tienda del Explorador</h2><p>Consigue objetos con las monedas ganadas en misiones reales.</p></Link> : <article className="world-zone shop-zone locked" aria-disabled="true"><span className="zone-icon" aria-hidden="true">🏪</span><span className="zone-state">🔒 NIVEL 2</span><h2>Tienda del Explorador</h2><p>Completa misiones para abrirla.</p></article>}
