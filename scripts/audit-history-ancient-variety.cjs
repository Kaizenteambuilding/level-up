...
assert(!/generated\s*\?\?=\s*generateCurriculumQuestion/.test(session),'History Archive must not knowingly fall back to a repeated prompt')
assert(session.includes('<strong>Por qué:</strong> {question.solution}'),'History Archive must expose pedagogical explanation')
console.log('History variety audit passed: 8+ structures per skill, strict anti-repeat and pedagogical explanation.')