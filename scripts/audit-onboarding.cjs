...
assert.ok(onboarding.includes("requested === 'reward'"), 'Reward route is not interpreted explicitly')
assert.ok(onboarding.includes('player.onboardingMissionCompleted'), 'Reward screen is not bound to a completed calibration mission')
assert.ok(onboarding.includes("router.replace('/world')"), 'Completed onboarding can be replayed through a direct route')
assert.ok(briefing.includes("'/mission?onboarding=1'"), 'Briefing drops onboarding state')
assert.ok(briefing.includes("allowIncompleteOnboarding: 'onboarding-query'"), 'Direct briefing route can bypass onboarding')
...
assert.ok(shell.includes("typeof avatar.onboarding_started_at === 'string'"), 'Onboarding query exception is not bound to avatar selection')
assert.ok(shell.includes('endedAt >= onboardingStartedAt'), 'Calibration completion is not derived from a post-avatar mission')
