...
for (const unit of curricula.SUBJECT_CURRICULA.english) {
  for (const definition of unit.skills) {
    ...
    if (prompts.size < 2) failures.push(`${definition.id}:only_${prompts.size}_unique_prompt`)
  }
}
...
console.log(JSON.stringify({ englishSkills: curricula.SUBJECT_CURRICULA.english.flatMap((unit) => unit.skills).length, generated, failures }, null, 2))
