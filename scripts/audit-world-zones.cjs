const assert = require('node:assert/strict')
const fs = require('node:fs')
const vm = require('node:vm')

let source = fs.readFileSync('lib/worldZones.ts', 'utf8')
source = source
  .replace(/export type[\s\S]*?\n\nconst comingSoon/, 'const comingSoon')
  .replace(/: WorldDistrict/g, '')
  .replace(/: WorldZone\[\]/, '')
  .replace(/export function/g, 'function')
  .replace(/value: string/, 'value') + '\nmodule.exports={WORLD_ZONES,worldZoneById}'
const box = { module: { exports: {} } }
vm.runInNewContext(source, box)
const { WORLD_ZONES, worldZoneById } = box.module.exports
assert.deepEqual(Array.from(WORLD_ZONES, (zone) => zone.id), ['language', 'english', 'science', 'creative'])
for (const zone of WORLD_ZONES) {
  assert.equal(zone.districts.length, 3, `${zone.id} must have three districts`)
  assert.equal(worldZoneById(zone.id).name, zone.name)
  for (const district of zone.districts) {
    assert.ok(['coming_soon', 'playable'].includes(district.availability), `${zone.id}/${district.name} must declare availability`)
    if (district.availability === 'playable') assert.ok(district.href?.startsWith('/'), `${zone.id}/${district.name} playable district must have an internal href`)
  }
}
assert.equal(worldZoneById('unknown'), undefined)

const language = worldZoneById('language')
assert.equal(language.districts[0].name, 'Galería de lectura')
assert.equal(language.districts[0].availability, 'playable')
assert.equal(language.districts[0].href, '/zone/language/reading')
assert.equal(language.districts[1].name, 'Taller de palabras')
assert.equal(language.districts[1].availability, 'playable')
assert.equal(language.districts[1].href, '/zone/language/words')
assert.equal(language.districts[2].availability, 'coming_soon')
assert.equal(language.districts.filter((district) => district.availability === 'playable').length, 2)

const english = worldZoneById('english')
assert.equal(english.districts[0].name, 'Muelle de escucha')
assert.equal(english.districts[0].availability, 'playable')
assert.equal(english.districts[0].href, '/zone/english/listening')
assert.equal(english.districts[1].name, 'Plaza de conversación')
assert.equal(english.districts[1].availability, 'playable')
assert.equal(english.districts[1].href, '/zone/english/conversation')
assert.equal(english.districts[2].name, 'Terminal de palabras')
assert.equal(english.districts[2].availability, 'playable')
assert.equal(english.districts[2].href, '/zone/english/terminal')
assert.equal(english.districts.filter((district) => district.availability === 'playable').length, 3)

const preview = fs.readFileSync('components/ZonePreview.tsx', 'utf8')
assert.ok(preview.includes('DISPONIBLE PRÓXIMAMENTE'), 'Preview must use the agreed upcoming label')
assert.ok(preview.includes("district.availability === 'playable'"), 'Preview must render district availability from data')
assert.ok(preview.includes('ENTRAR AL DISTRITO'), 'Playable districts must expose an entry action')
assert.ok(!preview.includes('EN PREPARACIÓN'), 'Legacy preparation label must not return')
const terminal = fs.readFileSync('components/EnglishTerminalSession.tsx', 'utf8')
assert.ok(terminal.includes("const MODE = 'english_terminal'"), 'English terminal must use its bounded practice mode')
const conversation = fs.readFileSync('components/EnglishConversationSession.tsx', 'utf8')
assert.ok(conversation.includes("const MODE = 'english_conversation'"), 'English conversation must use its own bounded practice mode')
const listening = fs.readFileSync('components/EnglishListeningSession.tsx', 'utf8')
assert.ok(listening.includes("const MODE = 'english_listening'"), 'English listening must use its own bounded practice mode')
assert.ok(listening.includes('SpeechSynthesisUtterance'), 'Listening district must synthesize English audio')
assert.ok(listening.includes("'english_listening'"), 'Listening attempts must be identifiable')
assert.ok(listening.includes('LISTEN: ${item.spoken}'), 'Listening evidence must persist the spoken source')
assert.ok(listening.includes('complete_levelup_session'), 'Listening district must close through authoritative completion')
const reading = fs.readFileSync('components/SpanishReadingSession.tsx', 'utf8')
assert.ok(reading.includes("const MODE = 'spanish_reading'"), 'Spanish reading must use its own bounded practice mode')
assert.ok(reading.includes('READING_SKILL_IDS'), 'Reading gallery must use a reading-focused skill subset')
assert.ok(reading.includes("'spanish_reading'"), 'Spanish reading attempts must be identifiable')
assert.ok(reading.includes('complete_levelup_session'), 'Spanish reading must close through authoritative completion')
const words = fs.readFileSync('components/SpanishWordsSession.tsx', 'utf8')
assert.ok(words.includes("const MODE = 'spanish_words'"), 'Spanish words must use its own bounded practice mode')
assert.ok(words.includes('WORD_SKILL_IDS'), 'Word workshop must use grammar and vocabulary skill subsets')
assert.ok(words.includes("'spanish_words'"), 'Spanish word attempts must be identifiable')
assert.ok(words.includes('complete_levelup_session'), 'Spanish word workshop must close through authoritative completion')
const guard = fs.readFileSync('components/MissionGuard.tsx', 'utf8')
assert.ok(guard.includes("mode === 'english_conversation'"), 'Mission guard must route conversation mode')
assert.ok(guard.includes("mode === 'english_listening'"), 'Mission guard must route listening mode')
assert.ok(guard.includes("mode === 'spanish_reading'"), 'Mission guard must route Spanish reading mode')
assert.ok(guard.includes("mode === 'spanish_words'"), 'Mission guard must route Spanish word workshop mode')
console.log('World zones audit passed.')
