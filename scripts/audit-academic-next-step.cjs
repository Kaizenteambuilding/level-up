...
assert.ok(component.includes('.limit(12)'), 'Recommendation must consider multiple candidates so inactive skills cannot mask an active priority')
...
assert.ok(component.includes('candidates.find'), 'Recommendation must preserve adaptive ordering while skipping inactive skills')
...
assert.ok(recommendation.includes("subjectId === 'biology_geology'"), 'Biology/Geology must use the persisted subject id')
assert.ok(recommendation.includes("subjectId === 'geography_history'"), 'Geography/History must use the persisted subject id')
assert.ok(!recommendation.includes("subjectId === 'science'"), 'Legacy science subject id must not drive persisted recommendations')
assert.ok(!recommendation.includes("subjectId === 'geography'"), 'Legacy geography subject id must not drive persisted recommendations')
