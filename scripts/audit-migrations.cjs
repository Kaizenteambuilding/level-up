...
for (const [version] of productionEntries) {
  assert.ok(mappedProductionVersions.includes(version), `Production migration has no repository provenance mapping: ${version}`)
}
