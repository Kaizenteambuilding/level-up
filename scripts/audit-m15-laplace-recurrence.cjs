...
const skillIndex = skills.findIndex((skill) => skill.id === 'M15S04')
if (skillIndex < 0) throw new Error('M15S04 no existe en el catálogo activo.')
...
if (templates.size < 24) failures.push(`only ${templates.size} normalized templates; expected at least 24`)
if (immediateRepeats > 4) failures.push(`${immediateRepeats} immediate repeats; expected at most 4`)
if (exactWithin[30] > 90) failures.push(`${exactWithin[30]} exact repeats within 30 samples; expected at most 90`)
...
if (failures.length) throw new Error(`M15S04 Laplace recurrence audit failed with ${failures.length} issue(s).`)
