...
const migration = fs.readFileSync('database/migrations/20260826200500_restore_xp_commit_on_completion.sql', 'utf8')
...
assert.ok(attempt.includes("join public.curriculum_units cu on cu.id = s.unit_id"), 'attempt RPC must preserve multi-subject curriculum validation')
assert.ok(attempt.includes("'reconciled', true"), 'attempt RPC must preserve lost-response reconciliation')
...
assert.ok(completion.includes('set xp = v_total_xp'), 'completion must persist committed mission XP')
assert.ok(completion.includes('for update'), 'completion must lock authoritative rows while committing rewards')

console.log('Mission XP commit audit passed against the latest authoritative migration.')
