...
const skillIndex = skills.findIndex((skill) => skill.id === 'M15S01')
if (skillIndex < 0) throw new Error('M15S01 no existe en el catálogo activo.')
...
if (templates.size < 26) failures.push(`only ${templates.size} normalized templates; expected at least 26`)
if (immediateRepeats > 3) failures.push(`${immediateRepeats} immediate repeats; expected at most 3`)
if (exactWithin[30] > 82) failures.push(`${exactWithin[30]} exact repeats within 30 samples; expected at most 82`)
...
if (failures.length) throw new Error(`M15S01 random experiment recurrence audit failed with ${failures.length} issue(s).`)
