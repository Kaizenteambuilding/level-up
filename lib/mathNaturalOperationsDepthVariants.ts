...
export function generateMathNaturalOperationsDepthVariant(skill:SkillMeta,difficulty:number,seed:number):GeneratedQuestion|null{
  ...
  else if(family===11){const q=a, r=c-1, dividend=q*b+r;prompt=`Al dividir ${dividend} entre ${b}, ¿cuál es el cociente entero?`;answer=String(q);distractors=[String(q+1),String(b),String(r)];solution=`${dividend} = ${b}×${q} + ${r}, con resto menor que ${b}; el cociente es ${q}.`}
  else if(family===12){const q=a,r=c-1,dividend=q*b+r;prompt=`Al dividir ${dividend} entre ${b}, ¿cuál es el resto?`;answer=String(r);distractors=[String(q),String(b),String(r+1)];solution=`${dividend} = ${b}×${q} + ${r}; por tanto el resto es ${r}.`}
  else if(family===13){const x=a*b;prompt=`¿Cuál de estas divisiones es exacta?`;answer=`${x} ÷ ${b}`;distractors=[`${x+1} ÷ ${b}`,`${x+2} ÷ ${b}`,`${x+b-1} ÷ ${b}`];solution=`${x} es múltiplo de ${b}, porque ${x}=${a}×${b}.`}
