...
function graph(seed: number): Item {
  ...
  if (family === 7) return { prompt:`Una gráfica usa divisiones de ${scale} en ${scale}. ¿Dónde debe situarse un valor ${scale * a}?`, answer:`En la marca ${scale * a}`, distractors:[`En la marca ${scale * (a - 1)}`,`En la marca ${scale * (a + 1)}`,`Entre 0 y ${scale} siempre`], solution:'La escala determina el valor representado por cada marca del eje.' }
