...
function e02s04(family: number, i: number) {
  ...
  if (family === 3) return item('E02S04',3,`${byIndex(NAMES,i)} is going to the cinema. The film begins at ${timeLabel(7 + i%2,30)}. We want to arrive twenty minutes early, and the ${byIndex(TRANSPORT,i)} journey takes fifteen minutes.`,`How many minutes before the film should they leave?`,`35 minutes`,['15 minutes','20 minutes','50 minutes'],`They need 20 minutes of early arrival plus 15 minutes of travel.`,i+3)
...
function e05s04(family: number, i: number) {
  ...
  if (family === 3) return item('E05S04',3,`At the ${place}, I would like the ${food}, please. Could I have bread instead of chips with it?`,`What change does the customer request?`,`Bread instead of chips`,['Chips instead of bread','Soup instead of bread','No side dish'],`The customer asks to replace chips with bread.`,i+3)
...
function e06s04(family: number, i: number) {
  ...
  if (family === 2) return item('E06S04',3,`At the ${place}, the café kitchen closes at ${timeLabel(3+i%2,0)}, although drinks are served until ${timeLabel(4+i%2,30)}.`,`What can a customer still order after the kitchen closes?`,`A drink`,[food,'A cooked meal','A hot sandwich'],`Food service has ended, but drinks continue later.`,i+2)
