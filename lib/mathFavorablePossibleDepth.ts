...
function item(seed:number):Item{
  ...
  const context=['una bolsa','una urna','una caja','un recipiente'][(seed>>>28)%4]
  const object=['bolas','fichas','tarjetas','piezas'][(seed>>>30)%4]
  if(family===0)return{prompt:`En ${context} hay ${red} ${object} rojas, ${blue} azules y ${green} verdes. ¿Cuántos casos posibles hay al sacar una bola si cada bola es un resultado elemental?`,answer:String(total),distractors:[String(red+blue),'3',String(total+1)],solution:`Hay ${total} bolas y cada una cuenta como resultado posible.`}
  if(family===1)return{prompt:`En ${context} hay ${red} ${object} rojas, ${blue} azules y ${green} verdes. ¿Cuántos casos favorables tiene “sacar roja”?`,answer:String(red),distractors:[String(total),String(blue),String(green)],solution:`Las ${red} bolas rojas son exactamente los casos favorables.`}
  if(family===2)return{prompt:`En ${context} hay ${red} ${object} rojas, ${blue} azules y ${green} verdes. ¿Cuántos casos favorables tiene “no sacar verde”?`,answer:String(red+blue),distractors:[String(green),String(total),String(red)],solution:'Son favorables todas las bolas rojas o azules.'}
  ...
  if(family===14)return{prompt:`En ${context} hay ${total} ${object} y el suceso A contiene ${red+green} de ellas. ¿Cuál es el número de casos posibles de A?`,answer:String(total),distractors:[String(red+green),String(red),String(green)],solution:'Los casos posibles pertenecen al experimento completo, no solo al suceso A.'}
  if(family===15)return{prompt:`En ${context} hay ${total} ${object} y ${blue} cumplen el suceso B. ¿Cuál es el número de casos favorables de B?`,answer:String(blue),distractors:[String(total),String(red+green),String(Math.max(1,blue-1))],solution:'Los favorables son exactamente los resultados que cumplen B.'}
  ...
  if(family===20)return{prompt:`En ${context} con ${red} ${object} rojas y ${blue} azules, ¿cuántos casos favorables tiene “roja o azul”?`,answer:String(red+blue),distractors:[String(red),String(blue),'2'],solution:'Como todas las bolas son rojas o azules, todos los resultados son favorables.'}
  return{prompt:`En ${context} con ${red} ${object} rojas, ${blue} azules y ${green} verdes, ¿cuántos casos favorables tiene “ni roja ni azul”?`,answer:String(green),distractors:[String(red+blue),String(total),String(red)],solution:'La única categoría que no es roja ni azul es verde.'}
}
