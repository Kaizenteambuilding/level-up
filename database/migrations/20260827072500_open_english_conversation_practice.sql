...
begin
  ...
  if p_mode not in ('english_terminal', 'english_conversation') then raise exception 'Practice mode is not available'; end if;
...
comment on function public.open_levelup_practice_session(uuid,text) is 'Opens or resumes bounded authenticated English district practice. Supports Terminal de palabras and Plaza de conversación, each limited to one completion per Madrid calendar day.';
