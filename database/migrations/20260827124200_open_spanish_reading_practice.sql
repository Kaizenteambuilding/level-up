-- Version: 20260827124200
-- Production version: 20260827124231
...
begin
  ...
  if p_mode not in ('english_terminal', 'english_conversation', 'english_listening', 'spanish_reading') then
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(p_player_id::text || ':' || p_mode, 0)
  );
...
comment on function public.open_levelup_practice_session(uuid,text) is 'Opens or resumes bounded authenticated district practice. Supports English Terminal, Conversation, Listening and Spanish Reading, each limited to one completion per Madrid calendar day.';
