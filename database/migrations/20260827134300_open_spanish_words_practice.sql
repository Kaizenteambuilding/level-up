-- Version: 20260827134300
-- Canonical repository migration; production applied as 20260827134312.
...
begin
  ...
  if p_mode not in ('english_terminal', 'english_conversation', 'english_listening', 'spanish_reading', 'spanish_words') then
    raise exception 'Practice mode is not available';
  end if;
  ...
  if exists (
    ...
    raise exception 'Practice already completed today';
  end if;
  ...
  if v_session_id is not null then
    ...
    values (p_player_id, p_mode, 'practice')
...
comment on function public.open_levelup_practice_session(uuid,text) is 'Opens or resumes bounded authenticated district practice. Supports English Terminal, Conversation, Listening and Spanish Reading/Words, each limited to one completion per Madrid calendar day.';
