-- Keep per-attempt XP provisional until a daily mission is completed.
-- Existing historical XP is preserved. At migration time production had no open sessions with attempts.
...
begin
  ...
  if not exists (
    ...
    where s.id = p_skill_id and s.active = true and s.generator_key is not null
      and s.unit_id = any (array['M01','M02','M03','M04','M05','M06','M07','M08','M09','M10','M11','M12','M13','M14','M15']::text[])
  ) then raise exception 'Skill is not available in the active curriculum'; end if;
  ...
  from public.player_skill_state where player_id=p_player_id and skill_id=p_skill_id for update;
  ...
  set mastery=v_mastery, confidence=v_confidence, difficulty=v_new_difficulty, priority=v_priority,
  ...
  insert into public.attempts(session_id,player_id,skill_id,question_seed,difficulty,prompt_snapshot,correct,response_ms,xp_awarded,diagnostic_tags,created_at)
  values(p_session_id,p_player_id,p_skill_id,p_seed,v_attempt_difficulty,p_prompt,p_correct,p_response_ms,v_xp_awarded,coalesce(p_diagnostic_tags,'{}'::text[]),now())
  returning id into v_attempt_id;

  return jsonb_build_object(
    'attempt_id',v_attempt_id,'attempt_number',v_attempt_count+1,'xp_awarded',v_xp_awarded,
    'xp',v_current_xp,'level',v_current_level,'mastery',v_mastery,'confidence',v_confidence,
    'difficulty',v_new_difficulty,'priority',v_priority
  );
end;
$function$;

revoke execute on function public.submit_levelup_attempt(uuid,text,boolean,integer,integer,bigint,text,uuid,text[]) from public,anon;
grant execute on function public.submit_levelup_attempt(uuid,text,boolean,integer,integer,bigint,text,uuid,text[]) to authenticated;
...
begin
  ...
  select ss.player_id, ss.completed, ss.ended_at into v_player_id,v_completed,v_ended_at
  ...
  if coalesce(v_completed,false) then
    ...
    return jsonb_build_object('completed',true,'attempts',v_attempt_count,'correct',v_correct,
      ...
      'level_before',v_level_before,'level_after',v_level_after,'actual_minutes',v_actual_minutes,'ended_at',v_finished_at);
  end if;
  ...
  set xp=v_total_xp, coins=coalesce(coins,0)+v_coins_earned, level=v_level_after
