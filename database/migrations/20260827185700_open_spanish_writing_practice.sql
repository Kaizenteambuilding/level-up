-- Version: 20260827185700
-- Canonical repository migration; production applied as 20260827185743.
...
begin
  ...
  if p_mode not in ('english_terminal','english_conversation','english_listening','spanish_reading','spanish_words','spanish_writing') then raise exception 'Practice mode is not available'; end if;
...
comment on function public.open_levelup_practice_session(uuid,text) is 'Opens or resumes bounded authenticated district practice, including Spanish Writing, limited to one completion per Madrid calendar day.';
